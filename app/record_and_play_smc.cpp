...
double smc(int error, int boundary, double k_v)
{
    if(error > -boundary && error < boundary)
        return error / (double)boundary * k_v;

    return error > 0 ? k_v : -k_v;
}
...
int main()
{
    ...
    FILE * log_file = fopen("./record_and_play_smc.log", "w");
